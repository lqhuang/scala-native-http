...
class HttpClientSuite extends munit.FunSuite {
  ...
  val emptyBodyHandler: HttpResponse.BodyHandler[String] =
  ...
  test("HttpClient should handle proxy settings") {
    val proxy = ProxySelector.of(InetSocketAddress.createUnresolved("proxy.example.com", 8080))
    val client = HttpClient.newBuilder().proxy(proxy).build()
    val request = HttpRequest.newBuilder(new URI("http://example.com")).GET().build()
    val resp = client.sendAsync(request, emptyBodyHandler).orTimeout(10, TimeUnit.SECONDS).get()
    assertEquals(resp.statusCode, 200)
  }
