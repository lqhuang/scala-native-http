...
import snhttp.jdk.net.internal.InMemoryCookieStore
...
class CookieManager(store: CookieStore, policy: CookiePolicy) extends CookieHandler:
  ...
  override def get(
      uri: URI,
      requestHeaders: JMap[String, JList[String]],
    ...
    Collections.sort(
      matchedCookies,
      (left: HttpCookie, right: HttpCookie) => compareCookiePath(left, right),
    )
                ...
                if (cookie.getPath() == null || cookie.getPath().isEmpty) {
                  ...
                  val defaultPath =
                    ...
                    then //
                      ...
                      if lastSlash <= 0 then "/" else path.substring(0, lastSlash + 1)
  ...
  private def compareCookiePath(left: HttpCookie, right: HttpCookie): Int = {
    ...
    if leftPath == rightPath then //
      ...
      if leftPath.startsWith(rightPath) then //
        -1
      else if rightPath.startsWith(leftPath) then //
        1
      else //
        0
    else 0
