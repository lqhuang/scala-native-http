...
class DelegateSeekableInputStream(
  ...
  override def seek(pos: Long): Boolean =
    if (pos < 0) || closed.get() || !isSeekable()
    then //
      false
    else {
      lock.lock()
  ...
  override def read(bytes: Array[Byte], offset: Int, length: Int): Int = {
    ...
    then //
      ...
      try
        ...
        do {
          val readBytes = upstream.read(bytes, offset + totalRead, length - totalRead)
