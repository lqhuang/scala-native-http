...
import scala.scalanative.unsafe.CLongInt
...
object KeyStoreUtils {
  ...
  private def createCertificate(
    ...
    NativeX509Support.createCertificate(
      publicKey,
      privateKey,
      subject,
      issuer,
  ...
  private def generateOCSPResponse(
    ...
    // OCSP response generation would require OpenSSL OCSP bindings (<openssl/ocsp.h>)
    //
    // - `OCSP_RESPONSE_new`
    // - `OCSP_BASICRESP_new`
    ...
    throw new NotImplementedError("OCSP response generation is not implemented")
  ...
  def createClient(caKeyStore: KeyStore): KeyStore = {
    ...
    copySelfSignedCertificates(clientKeyStore, caKeyStore): Unit
    clientKeyStore
  }
