...
enum CertificateStatus:
  case RevokedStatus(val revocationTime: Date, val reasonCode: Int) extends CertificateStatus
  case Good extends CertificateStatus
...
private[utils] object NativeX509Support:
  ...
  def createCertificate(
    ...
    val x509 = libcrypto.X509_new_ex(null.asInstanceOf[libcrypto.OsslLibCtxPtr], null)
    ...
    try {
      ...
      try {
        ...
        if (ca) addExtension(x509, "basicConstraints", critical = true, "CA:TRUE")

        val keyUsageValue = keyUsageString(keyUsage)
        if (keyUsageValue.nonEmpty) addExtension(x509, "keyUsage", critical = true, keyUsageValue)

        val extendedKeyUsageValue = extendedKeyUsages.asScala.map(_.value).mkString(",")
        if (extendedKeyUsageValue.nonEmpty)
          addExtension(
            x509,
            "extendedKeyUsage",
            criticalExtendedKeyUsages.asScala.exists(_ == true),
            extendedKeyUsageValue,
          )

        val subjectAltNameValue = subjectAltNames.asScala.map(_.toExtensionValue).mkString(",")
        if (subjectAltNameValue.nonEmpty)
        ...
        if (nameConstraintsValue.nonEmpty)
  ...
  private def addExtension(
    ...
    if (nid <= 0) throw new IllegalArgumentException(s"Unknown extension name: $name")
    val ext = libcrypto.X509V3_EXT_conf_nid(null, null, nid, toCString(value))
  ...
  private def decodePrivateKey(privateKey: PrivateKey)(using Zone): Ptr[libcrypto.EVP_PKEY] =
    ...
    try {
      val p8 = libcrypto.d2i_PKCS8_PRIV_KEY_INFO_bio(bio, null)
      try libcrypto.EVP_PKCS82PKEY(p8)
  ...
  private def decodePublicKey(publicKey: PublicKey)(using Zone): Ptr[libcrypto.EVP_PKEY] = {
    ...
    val bio = libcrypto.BIO_new_mem_buf(nativeBytes, encoded.length)
    try libcrypto.d2i_PUBKEY_bio(bio, null)
    finally libcrypto.BIO_free(bio): Unit
  }
  ...
  private def decodeX509Name(principal: X500Principal)(using Zone): Ptr[libcrypto.X509_NAME] = {
    ...
    !input = nativeBytes
    libcrypto.d2i_X509_NAME(null, input, encoded.length)
  }
  ...
  private def setSerialNumber(
    ...
    val serialBn = libcrypto.BN_bin2bn(nativeBytes, serialBytes.length, null)
    try {
      val asn1Serial = libcrypto.BN_to_ASN1_INTEGER(serialBn, libcrypto.X509_get_serialNumber(x509))
      if (asn1Serial != libcrypto.X509_get_serialNumber(x509))
  ...
  private def toJavaCertificate(x509: Ptr[libcrypto.X509])(using Zone): X509Certificate = {
    ...
    val bio = libcrypto.BIO_new(libcrypto.BIO_s_mem())
    try {
      libcrypto.i2d_X509_bio(bio, x509): Unit
