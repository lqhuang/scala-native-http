...
class ProxyTest extends utest.TestSuite {
  ...
  val tests = Tests {
     ...
     *
     * scala.scalanative.linker.LinkingException: Unreachable symbols found after classloading run.
     * It can happen when using dependencies not cross-compiled for Scala Native or not yet ported
     * JDK definitions.
     */
