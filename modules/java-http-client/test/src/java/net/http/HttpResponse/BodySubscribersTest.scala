...
class BodySubscribersTest extends TestSuite:
  ...
  val tests = Tests:
    ...
    test("fromSubscriber should forward onError to wrapped subscriber") {
      ...
      assert(mocker.lastError.isInstanceOf[RuntimeException])
      assertThrows[Exception] {
        subscriber.getBody().toCompletableFuture().get(): Unit
      }
    }
    ...
    test("limiting should handle multiple buffers in single onNext within capacity") {
      ...
      val result = subscriber.getBody().toCompletableFuture().get()
      assert(result.sameElements("HiBye".getBytes(StandardCharsets.UTF_8)))
    }
    ...
    // ======================================================= //
    // Additional ofByteArray edge cases                       //
    // ======================================================= //
    ...
    test("ofByteArray should handle ByteBuffer with non-zero position") {
      ...
      val result = subscriber.getBody().toCompletableFuture().get()
      assert(new String(result, StandardCharsets.UTF_8) == "Hello")
    }
    ...
    test("ofByteArray should handle empty ByteBuffer in the list") {
      ...
      val result = subscriber.getBody().toCompletableFuture().get()
      assert(result.sameElements("data".getBytes(StandardCharsets.UTF_8)))
    }

    // ======================================================= //
    // Additional ofByteArrayConsumer edge cases               //
    // ======================================================= //
    ...
    // ======================================================= //
    // Additional ofInputStream tests                          //
    // ======================================================= //
    ...
    // ======================================================= //
    // Additional fromLineSubscriber tests                     //
    // ======================================================= //
    ...
    test("fromLineSubscriber should not emit trailing empty line when body ends with separator") {
      ...
      assert(result == 2)
      assert(mocker.received.toSeq == Seq("line1", "line2"))
    }
    ...
    // ======================================================= //
    // Additional replacing / discarding edge cases            //
    // ======================================================= //
    ...
    test("discarding should return null specifically (Void)") {
      ...
      val result = subscriber.getBody().toCompletableFuture().get()
      assert(result == null)
      assert(result.asInstanceOf[AnyRef] eq null)
    }
