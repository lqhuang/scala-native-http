package snhttp.experimental.curl._internal

import scala.scalanative.unsafe.{Size, CInt, CLong, CSize}
import scala.scalanative.unsigned.UInt

private[curl] trait _BindgenEnumUInt[T](using eq: T =:= UInt):
...
private[curl] trait _BindgenEnumCLong[T](using eq: T =:= CLong):
  extension (inline t: T)
    inline def value: CLong = eq.apply(t)
    inline def int: Int = value.toInt

private[curl] trait _BindgenEnumCSize[T](using eq: T =:= CSize):
  extension (inline t: T)
    inline def value: CSize = eq.apply(t)
    inline def int: Int = value.toInt
    inline def long: Long = value.toLong

private[curl] trait _BindgenEnumLong[T](using eq: T =:= Long):
  extension (inline t: T) inline def value: Long = eq.apply(t)

private[curl] trait _BindgenEnumSize[T](using eq: T =:= Size):
  extension (inline t: T)
